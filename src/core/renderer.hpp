...
#define MAX_ANIMATED   64              // animated instances drawn per frame
...
static pix_renderer pix_create_renderer(int width, int height, idx shader, vec3 clear_color) {
    ...
    r.skinned_shader = opengl_create_shader(VSHDER_SKINNED, FSHDER_BASIC);
...
static idx load_material_image(pix_renderer& r, image_file_data* image, vec3 color, float metalic, float roughness) {
    idx id = (idx)r.material_count++;
    material& mat = r.materials[id];
    mat.shader = r.shader;
    mat.texture = 0;
    ...
    mat.roughness = roughness;
    if (image && image->data)
        mat.texture = opengl_create_texture2d(image->width, image->height, 4, image->data, TEXTURE_LINEAR);
    return id;
}
...
static void end_frame(pix_renderer& r, bool clear_instances) {
    ...
    // ---- animated pass ----
    // one draw per instance: each needs its own uBones upload, so there is
    // nothing to gain from instancing here
    if (r.animated_count && r.skinned_shader) {
        ...
        GLint u_color = glGetUniformLocation(r.skinned_shader, "uColor");
        GLint u_model = glGetUniformLocation(r.skinned_shader, "uModel");
        GLint u_bones = glGetUniformLocation(r.skinned_shader, "uBones[0]");
        if (u_bones < 0) u_bones = glGetUniformLocation(r.skinned_shader, "uBones");
    ...
    glBindVertexArray(0);
    if (clear_instances) { r.renderable_count = 0; r.animated_count = 0; }
}
